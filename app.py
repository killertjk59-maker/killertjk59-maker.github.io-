...
# ============ ТАНЗИМОТ ============
ADMIN_USERNAME = os.environ.get('ADMIN_USERNAME', 'Iskandar')
ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD', 'Fozilov Iskandar Yoribekovich')
...
DEFAULT_SETTINGS = {
    ...
    'china_name': 'Ном',
    'china_phone': '15757966666',
    'china_address': '义乌胜利达物流石塔头88幢2号电话',
    'china_note': '',
    'tj_address': 'ш. Истаравшон, рӯ ба рӯи "Манзили Сафо"',
    'tj_phone': '+992 98 909 11 11',
    'telegram_handle': 'Mir_express',
    'instagram_handle': 'Tajwaycargo',
}
...
<title>Tajwaycargo — Карго аз Чин ба Тоҷикистон</title>
...
Тел: {{ s.tj_phone }}</div>
  ...
  <footer>© 2026 Tajwaycargo. Ҳамаи ҳуқуқҳо ҳифз карда шудаанд.</footer>
</div>
...
<html lang="tg"><head><meta charset="UTF-8"><title>Воридшавӣ — Tajwaycargo Admin</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>
* { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI', Tahoma, sans-serif; }
...
<html lang="tg"><head><meta charset="UTF-8"><title>Панели идоракунӣ — Tajwaycargo</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>
* { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI', Tahoma, sans-serif; }
...
<body>
<div class="wrap">
  <h1>🔐 Tajwaycargo — Панели идоракунӣ
    <span>
      <a class="pill" href="/" target="_blank">🌐 Сайт</a>
      <a class="pill" href="/admin/telegram_status" target="_blank">🤖 Ҳолати бот</a>
  ...
  <div class="section">
    <h2>⚙️ Танзимоти пурра</h2>
    <form method="POST" action="/admin/settings">
      <h3 style="font-size:13px;color:#9aa4c0;margin:6px 0;">💰 Нархҳо ва мӯҳлат</h3>
      <div class="row">
        <input name="price_auto" type="number" step="0.1" value="{{ s.price_auto }}" placeholder="Нархи Авто (сом/кг)">
        <input name="delivery_auto_days" value="{{ s.delivery_auto_days }}" placeholder="Мӯҳлати Авто">
        <input name="price_avia" type="number" step="0.1" value="{{ s.price_avia }}" placeholder="Нархи Авиа (сом/кг)">
        <input name="delivery_avia_days" value="{{ s.delivery_avia_days }}" placeholder="Мӯҳлати Авиа">
      </div>
      <h3 style="font-size:13px;color:#9aa4c0;margin:14px 0 6px;">🇨🇳 Адреси анбори Хитой</h3>
      <div class="row">
        <input name="china_name" value="{{ s.china_name }}" placeholder="Ном">
        <input name="china_phone" value="{{ s.china_phone }}" placeholder="Телефон">
        <input name="china_address" value="{{ s.china_address }}" placeholder="Адрес (бо забони чинӣ)" style="flex:3;">
        <input name="china_note" value="{{ s.china_note }}" placeholder="Ёддошти иловагӣ">
      </div>
      <h3 style="font-size:13px;color:#9aa4c0;margin:14px 0 6px;">🇹🇯 Адреси Тоҷикистон</h3>
      <div class="row">
      ...
      <h3 style="font-size:13px;color:#9aa4c0;margin:14px 0 6px;">📱 Шабакаҳои иҷтимоӣ</h3>
      <div class="row">
        <input name="telegram_handle" value="{{ s.telegram_handle }}" placeholder="Telegram (бе @)">
        <input name="instagram_handle" value="{{ s.instagram_handle }}" placeholder="Instagram (бе @)">
      </div>
      <button class="act" type="submit" style="margin-top:12px;">💾 Захираи ҳамаи танзимот</button>
    </form>
  </div>
  ...
  <div class="section">
    <h2>📦📦 Илова кардани якчанд трек якбора</h2>
    <p style="color:#9aa4c0;font-size:12px;margin-bottom:10px;">Ҳар код дар сатри алоҳида нависед. Формат: <code>КОД</code> ё <code>КОД,мизоҷ,вазн</code></p>
    <form method="POST" action="/admin/tracks/bulk_add">
      <textarea name="codes" placeholder="AKEL2026010&#10;AKEL2026011,Азиз,4.2&#10;AKEL2026012" style="width:100%;min-height:100px;margin-bottom:10px;"></textarea>
      <div class="row">
        <select name="type"><option value="auto">🚚 Ҳамаашон Авто</option><option value="avia">✈️ Ҳамаашон Авиа</option></select>
        <select name="stage">
          {% for st in stages %}<option value="{{ loop.index0 }}">{{ st }}</option>{% endfor %}
        </select>
        <button class="act" type="submit">➕ Илова кардани ҳама</button>
      </div>
    </form>
  </div>
...
def admin_settings():
    s = load_settings()
    for key in ['price_auto', 'price_avia']:
        if request.form.get(key):
            s[key] = float(request.form.get(key))
    for key in ['delivery_auto_days', 'delivery_avia_days', 'china_name', 'china_phone',
                'china_address', 'china_note', 'tj_address', 'tj_phone',
...
@app.route('/admin/tracks/bulk_add', methods=['POST'])
@login_required
def admin_tracks_bulk_add():
    tracks = load_tracks()
    ttype = request.form.get('type', 'auto')
    stage = int(request.form.get('stage', 0))
    lines = request.form.get('codes', '').strip().splitlines()
    added = 0
    for line in lines:
        line = line.strip()
        if not line:
        ...
        weight = float(parts[2]) if len(parts) > 2 and parts[2] else 0
        tracks[code] = {
            'type': ttype, 'client': client, 'weight': weight, 'stage': stage,
...
def admin_telegram_status():
    ...
    if reset:
        setup_webhook()
        reset_result = tg('getWebhookInfo')
    return jsonify({
        'BOT_TOKEN_set': bool(BOT_TOKEN),
        'PUBLIC_URL': PUBLIC_URL,
        'ADMIN_TELEGRAM_IDS': ADMIN_TELEGRAM_IDS,
        'expected_webhook_url': f'{PUBLIC_URL}/telegram/webhook' if PUBLIC_URL else None,
        'current_webhook_info': info,
        'after_reset': reset_result,
        'hint': 'Барои аз нав танзим кардани webhook, ин суроғаро кушоед: /admin/telegram_status?reset=1',
    })
